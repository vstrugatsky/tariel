...
from loaders.dividends_from_polygon import LoadDividendsFromPolygon
...
def test_find_candidate_symbol():
    ...
    symbols = [symbol_1]
    assert (LoadDividendsFromPolygon.find_candidate_symbol(symbols, ex_dividend_date) == symbol_1)

    ex_dividend_date: date = datetime(2021, 11, 1).date()
    symbol_1 = Symbol(active=False, delisted=datetime(2016, 10, 7))
    symbols = [symbol_1]
    assert (LoadDividendsFromPolygon.find_candidate_symbol(symbols, ex_dividend_date) is None)
    ...
    symbols = [symbol_1, symbol_2]
    assert (LoadDividendsFromPolygon.find_candidate_symbol(symbols, ex_dividend_date) == symbol_2)
